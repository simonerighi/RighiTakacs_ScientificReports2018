% Run simulations to explore the effect of Probability of Forgiveness in four types of populations starting with:
...
for j=1:length(Prop_UD) % turn around the 4 lines
    ...
    for k=1:length(Pfor) % for each pfor
            
        clc
        display(['Experiment Number= ' num2str(j)])
        display(['Pfor= ' num2str(Pfor(k))])
        P_UD=Prop_UD(j);
        P_TFT=Prop_TFT(j);
        P_CR=Prop_CR(j);
        P_UR=Prop_UR(j);
        ...
        parfor i=1:Niter % 100 simulations
                ...
                [N_types(:,j,k,i),number_of_cooperators(j,k,i),number_of_plays(j,k,i),...
                    ...
                    P_CR,P_UR,P_UC,P_SJ,Pevo,Perr,Pfo,randseed(i),1,TFTtype,update_type);
        end
        save('NS_17_PforStudy_er.mat')
    end
end
