% Run simulations to explore the effect of Probability of Errors in four types of populations starting with:
...
Perr=[0:0.01:0.1]; % probability of committing errors
Pntw=NaN; % probability of random network change (IRRELEVANT)
Pfor=0; % forgiveness
...
for j=1:length(Prop_UD) % turn around the 4 lines
    for k=1:length(Perr) % for each pfor
        clc
        display(['Experiment Number= ' num2str(j)])
        display(['Perr= ' num2str(Perr(k))])
        P_UD=Prop_UD(j);
        P_TFT=Prop_TFT(j);
        P_CR=Prop_CR(j);
        P_UR=Prop_UR(j);
        ...
        parfor i=1:Niter % 100 simulations
                ...
                [N_types(:,j,k,i),number_of_cooperators(j,k,i),number_of_plays(j,k,i),...
                    ...
                    P_CR,P_UR,P_UC,P_SJ,Pevo,Per,Pfor,randseed(i),1,TFTtype,update_type);
        end
        save('NS_07_PerrStudy.mat')
    end
end
