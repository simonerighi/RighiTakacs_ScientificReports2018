% Run simulations to explore the effect of network density in two populations starting with all type of agents equally represented 
% OR with all type of agents equally represented except for TFT=0. The evolution is copy better.
...
for j=1:length(Prop_UD) % turn around the 4 lines
    for k=1:length(P_Link) % for each pevo
        ...
        P_Lin=P_Link(k);
            ...
            save('NS_21_Density_er_extended.mat')
