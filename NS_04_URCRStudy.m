...
% strategies, in presence  of SJ in the population, on a random network.
% Evolutionary strategy is copy the best.
...
for j=3:length(Prop_CR)
    if j==3
        stpos=6;
    else
        stpos=1;
    for k=stpos:length(Prop_UR)
        clc
        display(['CR= ' num2str(Prop_CR(j))])
        display(['UR= ' num2str(Prop_UR(k))])
